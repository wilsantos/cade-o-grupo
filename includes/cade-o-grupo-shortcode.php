...
function enqueue_backend_files()
{
   ...
   if (is_page('grupos') or is_page('reunioes-subcomite') or is_page('virtual')) {
    ...
    wp_enqueue_script('googlemaps', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyC7nYqPWJM9-6rVkh7d1VsdvtQqQafI4nQ&libraries=places', array(), '', true);
  }
	...
	if(isset($_GET['long'])){
		...
		if(!($_GET['A'] == "1" && $_GET['B'] == "1")){  //se não estiver checado os dois
			...
			if($_GET['formatos'] != "" && $_GET['ic_formato'] != "virtual"){
				...
				$uri .= "&formats[]=".$_GET['formatos']."&formats[]=56";
			}else if($_GET['formatos'] == "" && $_GET['ic_formato'] == "virtual"){
				$uri .= "&formats=56";
...
function getEstruturaGoogleMaps(){
	...
	$uri = "https://maps.googleapis.com/maps/api/geocode/json?address=".$searchGoogle."&key=AIzaSyC7nYqPWJM9-6rVkh7d1VsdvtQqQafI4nQ";
