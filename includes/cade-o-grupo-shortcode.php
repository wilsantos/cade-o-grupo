...

{
   ...
   if (is_page('grupos') or is_page('reunioes-subcomite') or is_page('virtual')) {
    ...
    wp_enqueue_script('googlemaps', 'https://maps.googleapis.com/maps/api/js?key='.get_option( 'chave_api_cade_o_grupo' ).'&libraries=places', array(), '', true);
	...
	if(isset($_GET['long'])){
				...
				if($ic_formato && ($reuniao->service_body_bigint != 142)){ //formato de reunião e tambem não traz um grupo de um CSA que foi excluido mas insiste em aparecer.
				   ...
				   if(isset($reuniao->location_info)){
					...
					if($_GET['bairro'] == ""){
						 ...
						 $colecaoReuniao->hasReuniao($nomeGrupo,$reuniao->location_neighborhood,$reuniao->location_municipality,$reuniao->start_time,$reuniao->weekday_tinyint,$reuniao->formats, $reuniao->location_street, $reuniao->location_sub_province,$reuniao->location_province,$reuniao->location_postal_code_1,$reuniao->location_nation,$reuniao->location_text . " " . $reuniao->comments,$reuniao->longitude,$reuniao->latitude, $reuniao->duration_time, $virtual_meeting_link,$virtual_meeting_additional_info,$location_info,$periodo);
		...
		if(isset($_GET['long'])){
			...
			foreach($colecaoReuniao->Reunioes as $reuniao){ 
				...
				//para reuniões hibridas que tem o mesmo nome:

				if(isset($arrReunioesPorNome[$reuniao->meeting_name])){

					$arrReunioesPorNome[$reuniao->meeting_name." #2"] = $arrReunioesPorNome[$reuniao->meeting_name];
...
function getEstruturaGoogleMaps(){
	...
	$uri = "https://maps.googleapis.com/maps/api/geocode/json?address=".$searchGoogle."&key=".get_option( 'chave_api_cade_o_grupo' );
